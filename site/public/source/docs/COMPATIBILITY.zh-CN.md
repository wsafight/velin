...
有效的 `0.5.x` 源码及其确定性运行行为在 `0.5.x` 补丁版本之间保持兼容；已经发布的 `0.4.0` 源码 fixture 也继续被接受。补丁版本可以增加语法或诊断，但不能静默改变已有合法程序的含义。
...
每个 `.velinc` artifact 都携带 `ARTIFACT_MAGIC` 和 `ARTIFACT_VERSION`。`0.5.x` 运行时继续读取版本 4 artifact，包括已经发布的 `0.4.0` fixture。解码器会在执行前拒绝未知版本、旧版本、损坏、超限或语义非法的 artifact。
...
执行策略扩展同样只追加：`VelinExecutionPolicy`、`velin_execution_policy_default`、`velin_machine_new_with_policy` 和取消函数都是新增导出；旧的 `velin_machine_new` 仍使用默认策略。已有结构体和函数的布局、所有权与行为不变，因此 ABI 版本仍为 1。
...
- `fixtures/compatibility/0.5.0/source.velin` 在发布前固定即将交付的可组合 DSL 行为，包括模块、纯函数、集合字面量、`for`、索引赋值和标量复合赋值。
...
安全修复覆盖当前次版本线的最新补丁，目前为 `0.5.x`。更早的次版本线和预发布版本不受支持，除非发布说明明确承诺。仓库提供私密安全报告通道时，应通过该通道报告问题。
