...
`ask` 回复栏是按顺序消耗的 JSON 数组。示例在字段为 `[1]` 时喝药水。用 `[0]` 走另一分支。字符串和布尔也合法：`["east"]`、`[true]`。

语言跟随站点（`?lang=zh` 或顶栏切换）。主题跟随 `localStorage` 里的 `velin-theme`。

## 浏览器里的限制

一次运行在 1,000 次宿主效果或 1 MiB 输出后停止。回复 JSON 上限 1 MiB。两次让出之间的立即 VM 步数仍是 10,000。见[资源预算](LIMITS.zh-CN.md)和 [WebAssembly](WASM.zh-CN.md)。
