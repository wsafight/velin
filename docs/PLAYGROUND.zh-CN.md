...
- **开始调试** 会检查源码并创建持久会话，在执行前暂停。
- **继续** 运行到下一个宿主效果边界或结束；**单步** 执行一条字节码操作并显示其源码行。
- **快照** 保存完整的可回放状态和当前输出。选择已保存分支并点 **恢复**，即可回到该状态、修改回复，并确定性地探索另一条分支。

普通运行会按顺序消耗 `ask` 回复栏中的 JSON 数组。调试器在每个效果边界停止，并为该边界使用当前数组的第一个回复，因此恢复快照后可以选择另一条分支。示例在字段为 `[1]` 时喝药水。用 `[0]` 走另一分支。字符串和布尔也合法：`["east"]`、`[true]`。畸形 JSON 或任意一个不支持的项会让整组回复被拒绝，后续答案不会错配到别的 `ask`。
