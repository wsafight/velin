...
## Phase one: execution policy and compatibility contracts

This phase stabilizes runtime boundaries before expanding the language.
...
- **Source language**: syntax and runtime behavior that remain compatible throughout `1.x`.
...
Completion criteria: every public boundary has a written compatibility policy, CI fixtures, and a deprecation process. Velin does not release `1.0` before these conditions hold.

## Phase two: typed host SDK

The goal is to make host-command integration feel close to ordinary Rust calls without placing native objects inside the VM.
...
## Phase three: a composable DSL

This phase addresses script reuse and organization without turning Velin into a general-purpose dynamic language.
...
## Phase four: complete domain tooling

The tooling should use the analyzability of a small language and host schemas to provide a more precise experience than a general dynamic language.
...
## Continuous work: performance and size evidence

Performance work spans every phase instead of becoming a final optimization sprint. Benchmarks have three groups:
...
`1.0` means hosts can rely on public contracts over time; it does not mean that Velin has as many features as another language. Before that release:

- Execution policy, host SDK, module, and pure-function semantics are settled.
- Source-language, Rust-facade, artifact, and C-ABI compatibility policies have survived release cycles.
- Old-version fixtures, property tests, differential tests, and cross-platform release gates remain green.
- Real embedders have exercised upgrades, cancellation, budgets, host failures, and artifact deployment.
- Security support, vulnerability reporting, and deprecation policies match actual releases.
- Performance and size baselines are reproducible across Rust, Wasm, and runtime-only embedding.
